      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {concepts.map((concept, index) => (
          <div
            ...
            tabIndex={0}
            className={`relative rounded-lg overflow-hidden border-2 transition-all duration-300 cursor-pointer group transform hover:scale-105 ${
              selectedConcept === concept ? 'border-brand-purple shadow-2xl shadow-purple-500/30 scale-105' : 'border-dark-border hover:border-brand-blue/50'
            }`}
            onClick={() => onSelect(concept)}
            onKeyDown={(e) => e.key === 'Enter' && onSelect(concept)}
          >
